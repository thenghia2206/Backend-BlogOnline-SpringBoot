...
public class PostService {
    ...
    @Transactional
    public void updateNumComment(Long id, PostDtoOutput postDto, Long change) {
        Post oldPost = postRepository.findById(id).orElseThrow(() -> new PostNotFoundException("For id " +id));
        Post post = mapFromDtoToNumComment(postDto, oldPost, change);
        postRepository.save(post);
    }
    //------------------------------------------------
    ...
    private PostDto mapFromPostToDto(Post post) {
        ...
        postDto.setNumComment((post.getNumComment()));
        return postDto;
    }
    ...
    private Post mapFromDtoToPost(PostDto postDto) {
        ...
        post.setThumnail(postDto.getThumnail());
        post.setCategory(categoryRepository.getOne(postDto.getCategory_id()));
        post.setNumComment(postDto.getNumComment());
        return post;
    }
    ...
    private Post mapFromDtoToNumComment(PostDtoOutput postDto, Post post, Long change) {
        post.setTitle(postDto.getTitle());
        post.setContent(postDto.getContent());
        post.setNumLike(postDto.getNumLike());
        post.setDesciption(postDto.getDescription());
        post.setThumnail(postDto.getThumnail());
        post.setCategory(categoryRepository.getOne(postDto.getCategory_id()));
        post.setNumComment(postDto.getNumComment()+change);
        return post;
    }
}
